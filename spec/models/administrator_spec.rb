...
describe Administrator do
  ...
  describe '#user' do  #we're in the Administrator class, and we're going to test the user instance method
    it 'has a user' do
      administrator = Administrator.new
      user = User.new
  ...
  describe '#metadata' do
    it 'has administrator properties' do
      administrator = Administrator.create(role:'db', ssn: '111-222-2323', tel: '212-555-1212')
      expect(administrator.id).to_not be nil
      expect(administrator.role).to eq 'db'
      expect(administrator.ssn).to eq '111-222-2323'
